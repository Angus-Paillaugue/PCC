...
function getProductId(url) {
  ...
  if (urlMatch(["*://*.pandabuy.com/*"], url)) {
    if ("itemID" == new URL(url)?.searchParams?.get("itemID"))
    ...
    if (urlMatch(["*://*.cssbuy.com//item-*.html"], location.href))
      return url.match(/item-[0-9]{12}/g)[0].replace("item-", "");
  }
...
function fetchLive(currencyToConvertTo, callback) {
  fetch(`https://pcc.paillaugue.fr/api/exchange-rates/${currencyToConvertTo}`)
    .then((res) => res.json())
    .then((data) => {
      chrome.storage.local.set(
        {
          cache: { currencyToConvertTo, data: data.rates },
          cacheTime: Date.now(),
        },
        () => {
          callback(data.rates);
...
function getRates(currencyToConvertTo, callback) {
  chrome.storage.local.get(["cache", "cacheTime"], ({ cache, cacheTime }) => {
    // Cache exists and is less than a day old
    if (
      cache &&
      cacheTime > Date.now() - 60 * 60 * 24 * 1000 &&
      cache?.currencyToConvertTo === currencyToConvertTo
    ) {
      console.log(`PCC : Using cached ${currencyToConvertTo} conversion rates`);
      return callback(cache.data);
    } else {
      console.log(
        `PCC : Fetching live ${currencyToConvertTo} exchange rates from api`,
      );
...
function formatString(inputString, rates, currencies, currencyToConvertTo) {
  ...
  if (
    ...
    (location.pathname === "/cartEstimatedFreight" ||
      location.pathname === "/person/parcel/list")
  ) {
  ...
  for (const match of matches) {
    ...
    if (currencySymbol && originalNumber) {
      ...
      if (currency.name !== currencyToConvertTo) {
        ...
        const convertedNumber = isNaN(originalNumber)
          ? ""
          : (parseFloat(originalNumber) / rate).toFixed(2);
        // Modifies the currency string to be converted to the chosen currency
        modifiedString +=
          inputString.substring(lastIndex, match.index - 1) +
          convertedNumber +
          " " +
          currencies.filter((el) => el.name === currencyToConvertTo)[0].symbol;
    ...
    getRates(convertTo, (rates) => {
      ...
      xhr.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status === 200) {
          ...
          if (titleEl)
            titleEl.innerText = formatString(
              titleEl.innerText,
              rates,
              currencies,
              convertTo,
            ...
            text.forEach((textNode) => {
              ...
              for (let i = 0; i < textNode.childNodes.length; ++i) {
                ...
                if (textNode.childNodes[i].nodeType === Node.TEXT_NODE) {
                  ...
                  if (formattedText !== textNode.childNodes[i].textContent)
  ...
  chrome.storage.local.get(
    ...
    ({ yupooInterfaceReDesign }) => {
      ...
      chrome.storage.local.get(
        ["yupooContentWidth"],
        ({ yupooContentWidth }) => {
          yupooContentWidth ??= 170;

          if (document.querySelector(".showalbumheader__main"))
            document.querySelector(".showalbumheader__main").style.maxWidth =
              "100%";
          if (document.querySelector(".showindex__gallerycardwrap"))
            document.querySelector(
              ".showindex__gallerycardwrap",
            ).style.maxWidth = "100%";
          if (document.querySelector(".showalbum__imagecardwrap"))
            document.querySelector(".showalbum__imagecardwrap").style.maxWidth =
              "100%";
          if (document.querySelector(".categories__box.clearfix"))
            document.querySelector(".categories__box.clearfix").style.maxWidth =
              "100%";
          ...
          const interval = setInterval(() => {
            ...
            if (imagesContainers.length > 0 || count === 10)
              clearInterval(interval);
            if (document.querySelector(".categories__box-right"))
              document.querySelector(
...
function toggleYupooSideBar() {
  ...
  chrome.storage.local.get(["removeYupooSideBar"], ({ removeYupooSideBar }) => {
    if (removeYupooSideBar) {
      chrome.storage.local.get(["yupooInterfaceReDesign"], (data) => {
        ...
        if (!yupooInterfaceReDesign) return;
        if (document.querySelector(".yupoo-categories-hide-sidebar"))
          document.querySelector(
            ".yupoo-categories-hide-sidebar",
          ).style.display = "none";
        if (document.querySelector(".categories__box-left"))
      ...
      if (document.querySelector(".yupoo-categories-hide-sidebar"))
        document.querySelector(".yupoo-categories-hide-sidebar").style.display =
          "block";
      if (document.querySelector(".categories__box-left"))
        document.querySelector(".categories__box-left").style.display = "block";
      if (document.querySelector(".categories__box-right"))
        document.querySelector(".categories__box-right").style.marginLeft =
          "216px";
    }
...
function customProductQC() {
  try {
    ...
    chrome.storage.local.get(["customProductQC"], ({ customProductQC }) => {
      if (customProductQC) {
        const productUrl = new URL(location.href).searchParams.get("url");
        let providerName = new URL(productUrl).origin
          .split(".")
          .at(-2)
          .split("/")
          .at(-1);
        ...
        const interval = setInterval(() => {
          ...
          try {
            ...
            $(".overview").after(
              `<div style="padding:20px; width:100%;" class="customProductQCContainer"><h2 class="margin-bottom:5px;">Product QC : </h2><iframe src="https://www.pandabuy.com/storageQcImg?itemId=${productID}&providerType=${providerType}&activeIndex=0&type=imageList" style="width:100%; aspect-ratio:16/9; border:none;" id="QCIframe"></iframe></div>`,
            );
...
function setDarkMode() {
  chrome.storage.local.get(["darkMode"], ({ darkMode }) => {
    if (darkMode && urlMatch(["*://*.pandabuy.com/*", "*://*.yupoo.com/*"])) {
      ...
      if (document.getElementById("PCCDarkTheme"))
        document
...
function track() {
  ...
  const parcelNumbers = document.querySelectorAll(
...
(function () {
  ...
  chrome.storage.local.get(["skipYupooRedirect"], ({ skipYupooRedirect }) => {
    if (
      skipYupooRedirect &&
      ...
      chrome.runtime.sendMessage({
        type: "updateTabURL",
        url: decodeURIComponent(new URL(location.href).searchParams.get("url")),
      });
    }
  });
  ...
  chrome.storage.local.get(
    ["autoPandaBuyRedirect"],
    ({ autoPandaBuyRedirect }) => {
      if (autoPandaBuyRedirect && isAProductPage(location.href)) {
        chrome.runtime.sendMessage({
          type: "updateTabURL",
