...
export async function load({ locals }) {
  ...
  const latestUpdate = {
    date: '2024-01-08',
    description: 'The PCC extension is now free! Enjoy :)',
  };
  const textualRates = [
    {
      name: 'AED',
  ...
  let exchangeRates = structuredClone(
    await exchangeRatesRef.findOne({ id: 1 })
  ).rates;
  exchangeRates = exchangeRates.map((rate) => {
    return {
      ...rate,
      symbol: textualRates.find(
        (textualRate) => textualRate.name === rate.currency
      ).symbol,
    };
  });
