...
export async function load({ locals }) {
    if(!locals?.user?.isAdmin) throw redirect(307, "/auth?redirect=/dashboard/admin/newsletter");
...
export const actions = {
    ...
    newMail: async({ request, locals }) => {
        ...
        if(!subject || !contents) return { success:false, formData, message:"Missing data!" };
