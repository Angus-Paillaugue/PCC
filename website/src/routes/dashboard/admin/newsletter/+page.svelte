<script>
  ...
  onMount(() => {
    const subscribersChart = new ApexCharts(
...
<section class="grow w-full p-4 lg:p-6 space-y-6">
  <div class="max-w-screen-lg mx-auto flex flex-col gap-6">
    <Button
      ...
      iconPosition="left"
      animationTo="rotate"
      icon="heroicons:arrow-uturn-left">Go back</Button
    >
    <div class="w-full h-full grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <Card class="relative">
        <h5
          class="w-full border-b border-neutral-200 dark:border-neutral-700 flex flex-row p-2 items-center justify-between"
        >
          Newsletter status
          <Button
            buttonType="button"
        ...
        <div
          class="bg-neutral-100 dark:bg-neutral-700 rounded-lg p-2.5 absolute top-0 left-0 right-0 transition-all {searchButton
            ? 'z-10 opacity-100'
            : '-z-10 opacity-0'}"
        >
          <TextInput
        ...
        <div
          class="flex flex-col overflow-y-auto overflow-x-hidden h-full max-h-56"
        >
          {#each subscribersArray as u}
            <form
              action="?/sendEmails"
              method="POST"
              class="w-full transition-all text-text-main dark:text-neutral-100 dark:hover:bg-neutral-600 hover:bg-neutral-100 p-1 md:p-2 cursor-pointer rounded-lg hover:px-4 items-center relative group flex flex-row justify-between"
              use:enhance={(e) => {
            ...
            >
              ...
              <div
                class="switch-wrapper opacity-0 group-hover:opacity-100 transition-all"
              >
                <input
                  type="checkbox"
                  id="sendEmail-{u.email}"
                  name="sendEmails-{u.email}"
                  checked={u.sendEmails}
                  class="hidden"
                  on:change={(e) => {
      ...
      <Card>
        <Button
          ...
          color="primary"
          on:click={() => {
            newNewsletterModal = true;
          }}>Send new newsletter</Button
        >
      </Card>
    </div>
  </div>
...
<Modal title="New newsletter mail" bind:status={newNewsletterModal}>
  ...
  >
    <p>
      In email subject and body, you can use use &#123;&#123;email&#125&#125 to
    ...
    <TextInput
      ...
      name="subject"
      id="subject"
      bind:value={newNewsletterFormData.subject}
    />
    ...
    <Textarea
      ...
      name="contents"
      id="contents"
      bind:value={newNewsletterFormData.contents}
    />
  </form>
  ...
  <svelte:fragment slot="footer">
    ...
    <Button
      ...
      buttonType="button"
      color="primary"
      disabled={newNewsletterFormData.subject?.length === 0 ||
        newNewsletterFormData.contents?.length === 0}
    >
