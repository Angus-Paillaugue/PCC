<script>
  ...
  function save(uuid) {
    ...
    const isPremium =
      userCard.querySelector(`[name="isPremium"]`).value == 'true'
        ? true
        : false;
    const isAdmin =
      userCard.querySelector(`[name="isAdmin"]`).value == 'true' ? true : false;
  ...
  function deleteAccount() {
    fetch(`/api/deleteUser`, {
      method: 'DELETE',
      body: JSON.stringify({ id: deleteAccountId }),
    })
      .then((res) => res.json())
      .then((res) => {
  ...
  function search() {
    ...
    usersArray = users
      .filter((u) =>
        searchQueryPremium == '*'
          ? true
          : searchQueryPremium == u.isPremium.toString()
      )
      .filter((u) => {
...
<section class="grow w-full p-4 lg:p-6 space-y-6">
  <div
    class="grid grid-cols-1 lg:grid-cols-3 md:grid-rows-2 gap-4 lg:gap-6 max-w-screen-2xl mx-auto"
  >
    ...
    <Card class="md:col-span-2">
      ...
      {#each credentials as credential}
        ...
        >
          <span class="text-center"
            >{credential.type} : {credential.username}</span
          >
          <button
            class="no-scale text-center"
            use:copy={credential.showPassword ? credential.password : ''}
            on:click={() => {
      ...
      <div class="flex flex-row gap-4 flex-wrap">
        ...
        >
          <a
            href="https://my.stripo.email/account/templates/1068358"
            target="_blank"
            class="link">E-mail templates</a
          >
        </Tooltip>
        <a
          href="https://vercel.com/angus-paillaugue/pcc"
          target="_blank"
          class="link">Vercel</a
        >
        <Tooltip
        ...
        >
          <a
            href="https://cloud.mongodb.com/v2/65663cc0b387d24672b14a2e#/clusters"
            target="_blank"
            class="link">MongoDB</a
          >
        </Tooltip>
        <a
          href="https://dash.cloudflare.com/92f5796f7fdf0c37529d5c39b8670c8f/paillaugue.fr"
          target="_blank"
          class="link">Cloudflare</a
        >
        <a
          href="https://analytics.google.com/analytics/web/?authuser=0#/p414534698/reports/intelligenthome"
          target="_blank"
          class="link">Analytics</a
        >
        <a
          href="https://insights.hotjar.com/sites/3748048/overview"
          target="_blank"
          class="link">Hotjar</a
        >
        <Tooltip text="Log-in with send.email.pcc@gmail.com google account">
  ...
  <Card class="max-w-screen-2xl mx-auto">
    ...
    {#if usersArray.length > 0}
      <div
        class="grid mt-2 gap-4"
        style="grid-template-rows: min-content; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));"
      >
        {#each usersArray.slice(0, noUsersToDisplay) as u}
          <div
            class="p-2 shadow-sm dark:shadow-none dark:border dark:border-neutral-700 rounded-lg flex flex-col gap-4"
            data-user-id={u.id}
          >
            <TextInput
              type="text"
            ...
            <p class="text-sm">
              Joined on : {new Date(u.joined).toLocaleString('fr-FR')}
            </p>
...
<Modal title="Delete account" bind:status={deleteAccountModal}>
  <p slot="body">
    Are you sure you want to delete {deleteAccountId
      ? users?.filter((el) => el.id === deleteAccountId)[0].username
      : ''}'s account ?
  </p>
  ...
  <svelte:fragment slot="footer">
    ...
    >
    <Button buttonType="button" color="primary" on:click={deleteAccount}
      >Yes, delete</Button
    >
  </svelte:fragment>
</Modal>
