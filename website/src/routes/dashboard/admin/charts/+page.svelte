<script>
    ...
    ...joinDates
      .filter(
        (date) =>
          new Date(date) >
          new Date(
            new Date().setDate(new Date().getDate() - (noDaysToDisplay + 2))
          )
      )
  ...
  Object.keys(userProgressionData)
    .reverse()
    .forEach((date) => {
  ...
  const getProgression = (nbDays) => {
    ...
    return Math.round(
      ((data.at(-1) - data.at(-nbDays)) / data.at(-nbDays)) * 100
    );
  };
  ...
  onMount(() => {
    ...
    usersChart.render();
    const premiumChart = new ApexCharts(
    ...
    <div class="w-full h-full grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card class="md:col-span-2 overflow-x-auto">
        <div class="w-full grid grid-cols-1 lg:grid-cols-3 p-4 gap-4">
          <div
            class="border border-neutral-200 dark:border-neutral-700 rounded-lg p-4 flex flex-row items-center justify-center"
          >
            <h3 class="text-center w-fit">
              +<span class="text-green-600">{getProgression(1)}</span>% Today
            </h3>
          </div>
          <div
            class="border border-neutral-200 dark:border-neutral-700 rounded-lg p-4 flex flex-row items-center justify-center"
          >
            <h3 class="text-center w-fit">
              +<span class="text-green-600">{getProgression(7)}</span>% Last 7
              days
            </h3>
          </div>
          <div
          ...
          >
            <h3 class="text-center w-fit">
              +<span class="text-green-600">{getProgression(20)}</span>% Last 20
              days
            </h3>
          </div>
        </div>
      ...
      <Card class="relative">
        ...
        <div class="flex flex-col gap-0 overflow-y-auto overflow-x-hidden">
          {#each usersArray as u}
            <div
              class="w-full transition-all text-text-main dark:text-neutral-100 dark:hover:bg-neutral-600 hover:bg-neutral-100 p-2 cursor-pointer rounded-lg hover:px-4 relative group flex flex-row justify-between"
            >
              {u.username}
