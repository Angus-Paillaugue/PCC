<script>
  ...
  const sections = [
    ...
    },
    {
      ...
      description: `Since the 2.1.0 version, the PCC extension is free.`,
  ...
  const features = [
    {
      ...
      description:
        'Tired of deciphering currency values while shopping on PandaBuy? PCC (PandaBuy Currency Converter) effortlessly parses PandaBuy pages, converting prices from CNY or other currencies into your preferred one. Stay updated with real-time exchange rates, ensuring a seamless and stress-free shopping experience.',
    },
    {
      title: 'Instant PandaBuy Access',
      description:
        'Say goodbye to the hassle of manually redirecting from marketplace pages to PandaBuy. PCC automates the process, seamlessly navigating you to the corresponding PandaBuy product page as soon as you land on a marketplace product. Save time and streamline your shopping journey.',
    },
    {
      title: 'Swift Checkout Checkbox',
      description:
        'Accelerate your checkout process on PandaBuy with PCC. The extension automatically checks the required checkbox when adding a product to your cart, eliminating the need for manual confirmation. Save valuable seconds and breeze through your purchases effortlessly.',
    },
    {
      title: 'Integrated Reviews and Photos',
      description:
        'Dive deeper into product details without navigating away. PCC embeds product reviews and photos directly into the page, providing a seamless browsing experience. Make informed decisions without the hassle of constantly switching between pages.',
    },
    {
      title: 'Customize Yupoo for Your Comfort',
      description:
        "Tailor your Yupoo experience with PCC's customization options. Toggle the sidebar on and off, and adjust the product grid width to suit your preferences. Enjoy a personalized browsing experience as you explore the vast array of products on Yupoo.",
    },
    {
      title: "Bypass Yupoo's Delay",
      description:
        'Experience zero delays when clicking on external links on Yupoo. PCC eliminates the standard 3-second delay, allowing you to navigate instantaneously to external links and swiftly discover more products without interruption.',
    },
    {
      title: 'Enchanting Dark Mode',
      description:
        "Elevate your visual experience on PandaBuy and Yupoo with PCC's Dark Mode. Enjoy a sleek and eye-friendly interface, reducing eye strain during extended shopping sessions. Immerse yourself in a stylish and comfortable browsing environment.",
    },
  ];
</script>
...
<svelte:head>
  ...
  <meta
    name="description"
    content="The one and only tool for PandaBuy shopping"
  />
</svelte:head>
...
{#if isPageLoaded}
  ...
  <section
    id="learn-more"
    class="p-4 md:p-6 lg:p-10 flex flex-col items-center justify-center gap-10 max-w-screen-xl w-full mx-auto"
  >
    <div class="w-full pt-24 flex flex-col gap-10">
      ...
      {#each sections as section, index}
        <div
          class="{index % 2 == 0
            ? 'row'
            : 'row-reverse'} grid lg:grid-cols-5 grid-cols-1 lg:grid-flow-col rounded-2xl lg:p-10 p-6 bg-neutral-100 dark:bg-gray-800 text-start transition-all"
          use:reveal={{ transition: 'fly', duration: 200, y: 60 }}
        >
          <div
            class="rounded-2xl lg:col-span-2 {index % 2 == 0
              ? 'lg:-mt-0 -mt-9 lg:-ml-20 -ml-9 lg:-mr-0 -mr-9 lg:col-start-1'
              : 'lg:-mt-0 -mt-9 lg:-ml-0 -ml-9 lg:-mr-20 -mr-9 lg:col-start-4'}"
          >
            <img
              src={section.imgSrc}
              alt={section.title}
              draggable="false"
              class="rounded-2xl w-auto h-full shadow-md {index % 2 == 0
                ? 'mr-auto'
                : 'ml-auto'} max-h-[300px]"
            />
          </div>
          ...
          <div
            class="flex flex-col w-full gap-4 justify-between lg:col-span-3 {index %
              2 ==
            0
              ? 'lg:pl-4 lg:col-start-3'
              : 'lg:pr-4 lg:col-start-1'}"
          >
    ...
    <div class="w-full pt-24 flex flex-col gap-10">
      ...
      <div class="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {#each features as feature}
          <div
            class="flex flex-col gap-4 rounded-lg bg-neutral-100 dark:bg-gray-800 cursor-default p-4 transition-all relative"
          >
            <!-- svelte-ignore a11y-no-static-element-interactions -->
            <div
              class="absolute top-0 left-0 w-full h-full border border-neutral-300 dark:border-gray-600 rounded-lg opacity-0 hover:opacity-100 transition-all"
              style="--mask: 135deg; mask-image: linear-gradient(var(--mask), #fff 0%, transparent 50%); -webkit-mask-box-image: linear-gradient(var(--mask), #fff 0%, transparent 50%);"
              on:mousemove={(e) => {
  ...
  <section
    class="max-w-screen-xl w-full mx-auto p-4 md:p-6 lg:p-10 items-center"
    id="newsletter"
  >
    <h4 class="text-primary-600 w-full font-extrabold">Newsletter</h4>
    <div
      class="w-full p-0 max-sm:pt-4 md:p-6 lg:p-10 flex flex-col md:flex-row gap-10 items-center"
    >
      <h2 class="max-lg:text-xl text-text-main dark:text-neutral-100">
      ...
      <div class="flex flex-col gap-4 w-full lg:w-1/2">
        ...
        {#if form?.type === 'newsletter'}
          <Alert
            type={form?.error === true ? 'error' : 'success'}
