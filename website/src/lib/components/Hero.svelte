<script>
    ...
    const smallestShoeSize = 39;
    let heroConversionRates;
    let convertTo;

    const jordanType = ["pure_money", "cool_gray", "pine_green", "seafoam", "thunder", "royalty"];
...
<!-- Hero -->
<div class="w-full min-h-[calc(100dvh-9rem)] h-full max-w-screen-2xl md:mx-auto p-2 my-10 flex flex-col">
    <div class="rounded-3xl overflow-hidden h-full grow relative dark:bg-gray-800 bg-neutral-100 flex flex-col justify-between">
        <div class="p-4 md:p-10 my-auto">
            <!-- Blob svg -->
            <svg viewBox="0 0 599 609" class="absolute top-0 left-0 -translate-y-1/3 -translate-x-1/3 max-md:w-full w-[600px] fill-primary-600 dark:fill-primary-900" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="md:max-w-[600px] md:w-[60%] relative flex flex-col gap-6 md:gap-14 bg-neutral-100/50 dark:bg-neutral-800/50 backdrop-blur-md p-6 rounded-lg md:shadow-md">
        ...
        <!-- Image column -->
        <div class="h-[80%] w-1/3 bg-primary-600 dark:bg-primary-900 absolute bottom-0 right-10 rounded-t-full flex flex-col items-center max-md:hidden">
            <img src="/illustrations/home_page/Hero/Jordan_4_{jordanType[Math.floor(Math.random()*jordanType.length)]}.webp" alt="Jordan 4 pure money" class="-rotate-12 w-full">
        </div>
        <!-- Bottom card -->
        <div class="dark:border-primary-900 border-2 border-primary-600 dark:bg-gray-900 bg-white p-4 rounded-3xl relative">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 relative mt-4">
                <!-- Without PCC -->
                <div class="p-4 flex flex-row gap-4 h-[240px] border dark:border-neutral-700 border-neutral-200 rounded-lg">
                    <img src="/illustrations/home_page/Hero/Air_force_1_white.png" alt="" class="rounded-lg my-auto md:max-h-full max-w-[50%]">
                    <div class="flex flex-col gap-1">
                        <h5>Air force 1 white</h5>
                        <p class="text-base font-semibold">CNY ¥ 160.0</p>
                        <div>
                            <p>Size :</p>
                            <div class="flex flex-row flex-wrap gap-1">
                                {#each { length : 6 } as _, i}
                                    <div>
                                        <input type="radio" name="size1" id="size1-{i+smallestShoeSize}" class="hidden peer" checked={i === randomSize} on:change={() => {document.getElementById(`size2-${i+smallestShoeSize}`).checked=true;}}>
                                        <label for="size1-{i+smallestShoeSize}" class="px-2 rounded-md cursor-pointer transition-all bg-neutral-200 dark:bg-neutral-700 hover:bg-neutral-300 dark:hover:bg-neutral-600 peer-checked:bg-neutral-400 peer-checked:hover:bg-neutral-400">{i+smallestShoeSize}</label>
                                    </div>
                                {/each}
                            </div>
                        </div>
                ...
                <div class="p-4 flex flex-row gap-4 h-[240px] border dark:border-neutral-700 border-neutral-200 rounded-lg">
                    ...
                    <div class="flex flex-col gap-1">
                        <h5>Air force 1 white</h5>
                        {#key heroConversionRates} 
                            <p class="text-base font-semibold" in:fade>{heroConversionRates?.currency} {heroConversionRates?.symbol} {(160/heroConversionRates?.rates["CNY"]).toFixed(2)}</p>
                        {/key}
                        <div>
                            <p>Size :</p>
                            <div class="flex flex-row flex-wrap gap-1">
                                {#each { length : 6 } as _, i}
                                    <div>
                                        <input type="radio" name="size2" id="size2-{i+smallestShoeSize}" class="hidden peer" checked={i === randomSize} on:change={() => {document.getElementById(`size1-${i+smallestShoeSize}`).checked=true;}}>
                                        <label for="size2-{i+smallestShoeSize}" class="px-2 rounded-md cursor-pointer transition-all bg-neutral-200 dark:bg-neutral-700 hover:bg-neutral-300 dark:hover:bg-neutral-600 peer-checked:bg-neutral-400 peer-checked:hover:bg-neutral-400">{i+smallestShoeSize}</label>
                                    </div>
                                {/each}
                            </div>
                        </div>
