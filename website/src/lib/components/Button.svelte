<script>
  ...
  const baseButtonClasses = {
    primary:
      'text-white bg-primary-600 dark:bg-primary-700 hover:bg-primary-700 dark:hover:bg-primary-800 disabled:bg-primary-400 dark:disabled:bg-primary-500 shadow-primary-700 dark:shadow-primary-500 hover:shadow-md hover:dark:shadow-sm inline-flex flex-row items-center justify-center gap-2 font-semibold transition-all duration-200 hover:rounded-lg rounded-3xl focus:outline-none disabled:cursor-not-allowed relative overflow-hidden',

    'primary-dark':
      'text-white bg-primary-700 dark:bg-primary-800 hover:bg-primary-800 dark:hover:bg-primary-900 disabled:bg-primary-500 dark:disabled:bg-primary-600 shadow-primary-800 dark:shadow-primary-600 hover:shadow-md hover:dark:shadow-sm inline-flex flex-row items-center justify-center gap-2 font-semibold transition-all duration-200 hover:rounded-lg rounded-3xl focus:outline-none disabled:cursor-not-allowed relative overflow-hidden',
  ...
  const finalButtonClasses = twMerge(
    baseButtonClasses[color],
    props.class,
    buttonSizes[size],
    buttonAnimations[animationTo]
  );
  const finalIconClasses = twMerge(
    baseIconClasses,
    iconAnimations[animationTo],
    iconClasses
  );
  delete props.class;
  delete props.href;
...
{#if buttonType === 'button'}
  <button {...props} class={finalButtonClasses} on:click {disabled}>
    {#if iconPosition === 'right'}
      <slot />
    {/if}
    {#if animationTo === 'grow'}
      <svg
        class="relative w-6 h-6 transition-all group-hover:translate-x-[4px]"
        viewBox="0 0 16 16"
        fill="none"
      >
        <path
        ...
        ></path>
        <path
          class="transition-all group-hover:[stroke-dashoffset:20] [stroke-dasharray:10] [stroke-dashoffset:10]"
          stroke="currentColor"
          d="M1.75 8H11"
          stroke-width="1.5"
    ...
    {:else if icon}
      <Icon {icon} class={finalIconClasses} />
    {/if}
...
{:else}
  <a {...props} {href} class={finalButtonClasses} on:click {disabled}>
    {#if iconPosition === 'right'}
      <slot />
    {/if}
    {#if animationTo === 'grow'}
